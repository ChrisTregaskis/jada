...
class ApplicationModal extends React.Component {
    ...
    displayData = (application) => {
        ...
        if (application.session_id === undefined) {
        ...
        let sessionTime = application.session_time.substring(0,5);
        let applied = application.applied.toString().toUpperCase();
        let desired = application.desired.toString().toUpperCase();
        ...
        displayData.push(
            <ul key={key}>
                ...
                <li key={key + 4}><span className="applicationDetail">Processed at:</span> {sessionTime}</li>
                ...
                <li key={key + 7}><span className="applicationDetail">Desired:</span> {desired}</li>
                <li key={key + 8}><span className="applicationDetail">Applied:</span> {applied}</li>
    ...
    displayApplicationData = () => {
        ...
        if (application !== undefined) {
            let applicationHTML = this.displayData(application.response.application)
            displayData.push(applicationHTML)
        }
        return displayData
    }
